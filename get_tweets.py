...
df = pd.read_csv(f'H:/マイドライブ/調査資料/{target_folder}/{user_name}.csv',encoding="utf-8")


date = df.sort_values('date',ascending=False).loc[1,'date']
date = date[0:10]
...
for i,tweet in enumerate(sntwitter.TwitterSearchScraper(f'from:{user_name} since:{date}').get_items()):
...
tweets_df.to_csv(f'tmp/{user_name}_tmp.csv',encoding="utf-8",index = False)

df2 = pd.read_csv(f'tmp/{user_name}_tmp.csv',encoding="utf-8",)

tweets_df = pd.concat([df, df2],ignore_index=True)
